...
from drum_patts import patterns, slow_patts, fast_patts, even_patts, uneven_patts
...
with mido.open_input("AKM320") as inport:
    ...
    # connect to sphero
    all_toys = scanner.find_toys(toy_names=[SPHERO_4])
    print(all_toys)
    toy = all_toys[0]
    ...
    with SpheroEduAPI(toy) as ball:
        ...
        while(True):
            ...
            if (curr_sec != prev_sec and default_on):
                prev_sec = curr_sec
                select = random.choice([c for c in YELLOWS if c != last])
                last = select
            ...
            if last_midi == 59:
                ...
                if (ball.get_acceleration()['z'] <= 0.35) or ((curr_time_check - last_time_check) > TIME_SINCE_COLLISION):
                    ...
                    if (idx >= patt_len):
                        ...
                        # read the latest pattern index from the csv
                        with open('pattern.csv', "r", encoding="utf-8", errors="ignore") as scraped:
                            update = scraped.readlines()[-1]

                        # depending on leader_idx, possible patterns change
                        leader_idx = int(update)
                        if (leader_idx <= 6):
                            my_patterns = [fast_patts, uneven_patts]
                        elif (leader_idx <= 12):
                            my_patterns = [even_patts, slow_patts]
                        elif (leader_idx <= 16):
                            my_patterns = [slow_patts, uneven_patts]
                        else:
                            my_patterns = [even_patts, fast_patts]

                        # choose a possible pattern and reset idx
                        selected_patterns = random.choice(my_patterns)
                        patt = random.choice(selected_patterns)
                        idx = 0
                        patt_len = len(patt)
